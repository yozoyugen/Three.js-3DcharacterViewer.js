			...
			this.rotateCamera = function () {
				...
				return function rotateCamera() {
					...
					if ( angle ) {
						...
						//console.log("2::offset_:%o", offset_);
						offset_.applyQuaternion( quaternion );
						//console.log("3::offset_:%o", offset_);

						scope.object.position.x += offset_.x;
						scope.object.position.y += offset_.y;
						scope.object.position.z += offset_.z;
						scope.target.x += offset_.x;
						scope.target.y += offset_.y;
						scope.target.z += offset_.z;

					} else if ( ! scope.staticMoving && _lastAngle ) {
						//console.log("! scope.staticMoving && _lastAngle");
/*
...
*/
			...
			this.panCamera = function () {
				...
				return function panCamera() {
					...
					if ( mouseChange.lengthSq() ) {
						...
						//console.log("scope.object.position:%o", scope.object.position);
						//console.log("scope.target:%o", scope.target);
			...
			this.update = function () {
				...
				if ( ! scope.noRotate  && _state == STATE.ROTATE ) {

					scope.rotateCamera();

				}

				if ( ! scope.noZoom  ) { //&& _state == STATE.ZOOM

					scope.zoomCamera();

				}

				if ( ! scope.noPan && _state == STATE.PAN ) {
